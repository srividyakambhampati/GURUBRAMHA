      ...
      <div className="max-w-[1600px] mx-auto px-8 md:px-12 pt-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-4 space-y-10">
                ...
                >
                    <h3 className="text-xl font-black text-slate-900 mb-10 flex items-center gap-5">
                        <FileCheck size={24} className="text-indigo-600" /> Vault Architecture
                    </h3>
                    <div className="space-y-10">
                        <div>
                            <div className="flex justify-between items-end mb-4">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Storage Capacity</p>
                                <p className="text-sm font-black text-slate-900 tracking-tight">
                                    {(documents.reduce((acc, doc) => acc + parseFloat(doc.size), 0)).toFixed(1)} <span className="text-slate-400">/ 100 MB</span>
                                </p>
                            </div>
                            <div className="h-4 w-full bg-slate-50 rounded-full overflow-hidden border border-slate-100 p-1">
                                <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full transition-all duration-700" style={{ width: `${(documents.reduce((acc, doc) => acc + parseFloat(doc.size), 0) / 100) * 100}%` }}></div>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            <div className="bg-slate-50 p-6 rounded-[24px] border border-slate-100 text-center">
                                ...
                                <p className="text-2xl font-black text-green-600 tracking-tight">{documents.filter(d => d.status === 'Uploaded').length}</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
            ...
            <div className="lg:col-span-8">
                ...
                >
                    <div className="absolute top-[-5%] right-[-5%] w-96 h-96 bg-indigo-50 rounded-full blur-[100px] opacity-40"></div>
                    ...
                    <div 
                        ...
                        className={`relative z-10 border-4 border-dashed rounded-[48px] p-16 transition-all flex flex-col items-center justify-center text-center cursor-pointer mb-16 ${
                            isDragging 
                                ? 'border-[#FFB800] bg-indigo-50/50 scale-[0.98]' 
                                : 'border-slate-100 bg-slate-50/30 hover:border-indigo-400 hover:bg-slate-50/50'
                        }`}
                    >
                        <div className="w-24 h-24 bg-white rounded-[32px] flex items-center justify-center text-indigo-600 mb-8 shadow-2xl group-hover:scale-110 transition-transform">
                            <Upload size={36} />
                        </div>
                        <h3 className="text-3xl font-black text-slate-900 mb-4 tracking-tight">Expand the Vault</h3>
                        <p className="text-slate-500 font-bold max-w-sm text-lg leading-relaxed">Establish security for your Resume, Aadhaar, or Degree credentials.</p>
                    </div>

                    <div className="relative z-10 space-y-10">
                        <div className="flex items-center justify-between border-b border-slate-50 pb-8">
                            <h3 className="text-2xl font-black text-slate-900 tracking-tight">Academic Manifest</h3>
                            <div className="flex items-center gap-4">
                                <span className="px-5 py-2 bg-slate-900 text-[#FFB800] rounded-full text-[10px] font-black uppercase tracking-widest">{documents.length} Entities</span>
                            </div>
                        </div>
                        
                        {documents.length === 0 ? (
                            <div className="py-32 text-center">
                                <div className="w-24 h-24 bg-slate-50 rounded-[32px] flex items-center justify-center text-slate-200 mx-auto mb-8">
                                    <FileText size={44} />
                                </div>
                                <p className="text-slate-400 font-black uppercase text-[10px] tracking-[0.4em]">The Vault remains empty</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {documents.map((doc) => (
                                    <motion.div 
                                        ...
                                        layout
                                        whileHover={{ y: -5 }}
                                        className="bg-slate-50 p-8 rounded-[40px] border border-slate-100 shadow-sm hover:shadow-2xl hover:bg-white transition-all group"
                                    >
                                        <div className="flex items-center justify-between mb-8">
                                            <div className="w-16 h-16 bg-white rounded-[24px] flex items-center justify-center text-slate-400 group-hover:text-indigo-600 transition-all shadow-sm">
                                                {['PDF', 'RESUME'].includes(doc.type) ? <FileText size={28} /> : 
                                                 ['PNG', 'JPG', 'JPEG'].includes(doc.type) ? <Upload size={28} /> : 
                                                 <CreditCard size={28} />}
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <button onClick={() => handleAction('share', doc)} className="p-3 bg-white rounded-2xl text-slate-400 hover:text-indigo-600 shadow-sm transition-all"><Share2 size={18} /></button>
                                                <button onClick={() => handleAction('delete', doc)} className="p-3 bg-white rounded-2xl text-red-400 hover:bg-red-50 shadow-sm transition-all"><Trash2 size={18} /></button>
                                            </div>
                                        </div>
                                        <div className="mb-10">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <button onClick={() => handleAction('view', doc)} className="flex items-center justify-center gap-3 py-4 bg-white text-slate-600 rounded-[20px] text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all shadow-sm">
                                                <Eye size={16} /> Inspect
                                            </button>
                                            <button 
                                                id={`dl-${doc.id}`}
                                                onClick={() => handleAction('download', doc)} 
                                                className="flex items-center justify-center gap-3 py-4 bg-indigo-600 text-white rounded-[20px] text-[10px] font-black uppercase tracking-widest hover:shadow-2xl shadow-indigo-200 transition-all min-w-[120px]"
                                            >
        ...
        {viewingDoc && (
            ...
            >
                <motion.div 
                    initial={{ scale: 0.95, y: 30 }}
                    animate={{ scale: 1, y: 0 }}
                    className="relative w-full max-w-6xl bg-white rounded-[56px] shadow-[0_40px_100px_rgba(0,0,0,0.5)] overflow-hidden border border-white/20 p-12 md:p-16 h-[90vh] flex flex-col"
                >
                    <button onClick={() => setViewingDoc(null)} className="absolute top-10 right-10 p-4 text-slate-400 hover:text-indigo-600 hover:bg-slate-50 rounded-[20px] transition-all z-20 shadow-sm"><X size={32} /></button>
                    
                    <div className="flex items-center gap-10 mb-16 flex-shrink-0">
                        <div className="w-24 h-24 bg-slate-900 rounded-[32px] flex items-center justify-center text-[#FFB800] shadow-2xl">
                            {['PNG', 'JPG', 'JPEG'].includes(viewingDoc.type) ? <Upload size={40} /> : <FileText size={40} />}
                        </div>
                        <div>
                            <h2 className="text-4xl font-black text-slate-900 tracking-tight">{viewingDoc.name}</h2>
                    ...
                    <div className="flex items-center justify-end gap-6 mt-16 flex-shrink-0">
                        <button onClick={() => setViewingDoc(null)} className="px-10 py-5 text-slate-400 font-black uppercase text-[10px] tracking-[0.2em] hover:text-slate-900 transition-all">Exit Inspector</button>
                        <button onClick={() => handleAction('download', viewingDoc)} className="px-12 py-5 bg-indigo-600 text-white rounded-[24px] font-black uppercase text-[10px] tracking-[0.3em] shadow-2xl shadow-indigo-200 hover:scale-[1.05] transition-all">Secure Download</button>
                    </div>
                </motion.div>
            </motion.div>
        )}
